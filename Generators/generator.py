...
def generateSamples(sceneNum, lightNum):
	...
	for sceneArrangements in range(0, sceneNum):
		...
		for lightArrangements in range(0, lightNum):
			baselineName = "\"/Users/Max/Dropbox (Brown)/CS2951/Training Data/PositionAlterationTest/baseline/baseline." + '%04d' % (sceneArrangements * lightNum + lightNum) + '.exr\"'
			beautyName = "\"/Users/Max/Dropbox (Brown)/CS2951/Training Data/PositionAlterationTest/beauty/beauty." + '%04d' % (sceneArrangements * lightNum + lightNum) + '.exr\"'
			nName = "\"/Users/Max/Dropbox (Brown)/CS2951/Training Data/PositionAlterationTest/N/N." + '%04d' % (sceneArrangements * lightNum + lightNum) + '.exr\"'
			zName = "\"/Users/Max/Dropbox (Brown)/CS2951/Training Data/PositionAlterationTest/Z/Z." + '%04d' % (sceneArrangements * lightNum + lightNum) + '.exr\"'
			randomizePointLight(initPosition)
			cmds.hide("point")
			cmds.showHidden("baseline")
